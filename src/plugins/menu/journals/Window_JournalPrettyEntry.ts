...
export default Window_JournalPrettyEntry.prototype = Object.create(Window_JournalEntry.prototype);
...
Window_JournalPrettyEntry.prototype.drawDetails = function() {
    ...
    new Promise(readyCheck).then(() => {
            ...
            for (let i = 0; i < this._paragraphs.length; i++) {
                ...
                if (paragraph.image || i === this._paragraphs.length - 1) {
                    ...
                    if (paragraph.image) {
                        ...
                        const bitmap: Bitmap = this.loadImage(paragraph.image);
        ...
        for (let image of displayedImages) {
            const bitmap: Bitmap = this.loadImage(image.name);
            this.contents.blt(
                bitmap,
                0, 0,
